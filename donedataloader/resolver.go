...
func (r *channelResolver) CreatedBy(ctx context.Context, obj *Channel) (*User, error) {
	...
	*/
	return ctx.Value(userLoaderKey).(*UserLoader).Load(obj.CreatedByID)
}
...
func (r *messageResolver) Channel(ctx context.Context, obj *Message) (*Channel, error) {
	/*
		...
		cs, err := r.DB.ListChannelsByID(ctx, []int{cID})
		if err != nil {
		...
		if len(cs) != 1 {
			return nil, fmt.Errorf("unexpected number of channels returned: %d", len(cs))
		}
	...
	*/
	return ctx.Value(channelLoaderKey).(*ChannelLoader).Load(obj.ChannelID)
}

type mutationResolver struct{ *Resolver }

func (r *mutationResolver) CreateMessage(ctx context.Context, input NewMessage) (*Message, error) {
	cID, err := strconv.Atoi(input.ChannelID)
	if err != nil {
		return nil, err
	}
	...
	err = r.DB.CreateMessage(ctx, &m)
	if err != nil {
		log.Printf("db error creating message: %#v", err)
		return nil, err
	}
