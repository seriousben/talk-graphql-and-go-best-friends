...
func (l *ChannelLoader) Load(key string) (*Channel, error) {
...
func (l *ChannelLoader) LoadThunk(key string) func() (*Channel, error) {
	l.mu.Lock()
...
func (l *ChannelLoader) LoadAll(keys []string) ([]*Channel, []error) {
	results := make([]func() (*Channel, error), len(keys))
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *ChannelLoader) LoadAllThunk(keys []string) func() ([]*Channel, []error) {
	results := make([]func() (*Channel, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *ChannelLoader) Prime(key string, value *Channel) bool {
	l.mu.Lock()
	var found bool
...
func (l *ChannelLoader) unsafeSet(key string, value *Channel) {
	if l.cache == nil {
		l.cache = map[string]*Channel{}
	}
	l.cache[key] = value
}
